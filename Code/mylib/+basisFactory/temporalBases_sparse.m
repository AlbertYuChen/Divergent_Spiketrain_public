...
[T, dx] = size(X);          %  X就是spike time 的稀疏形式,X=1000*1
[TB, M] = size(bases);      %  bases是926*10
...
featuresPerRow = sum(indices, 2); % (dx x 1) # of features per each event       %等于10
nTotalFeatuers = sum(featuresPerRow);                                           %等于10
featureRowIndex = [0; cumsum(featuresPerRow)];                                  %等于竖着的{0,10}
...
for k = 1:dx
    ...
    for kk = 1:length(idx)
        ...
        featureIdx = (featureRowIndex(k)+1):featureRowIndex(k+1);               %等于1到10
