function gg = makeFittingStruct_GLM_Goldfish(dtStim,dtSp,klength,nkbasis,k0,nhbasis,lasthpeak)
...
gg.ihbas_orig = []; % basis for h filter
...
if (nargin > 4) && (~isempty(k0)) && nkbasis ~= 0
...
if (nargin > 5) && (nhbasis>0)
    ...
    ihbasprs.b = lasthpeak/15;  % 15 JCNS Goldfish AIC BIC plot
    ihbasprs.absref = []; % absolute refractory period (optional)
    [iht, ihbas, ihbas_orig] = makeBasis_PostSpike(ihbasprs,dtSp);
    gg.iht = iht;
    gg.ihbas = ihbas;
    gg.ihbas_orig = ihbas_orig;
